...
class OrderController extends Controller
{
    ...
    {
        $order = Order::with('ordersitecontact', 'ordertimeslot', 'tickets')->findOrFail($id);
        ...
        // Fetch truck IDs that are in backlog
        $backloggedTrucks = Backlog::where('status', 'pending')->pluck('truck_id')->toArray();

        return view('backend.orders.show', compact('order', 'trucks', 'lastTimeSlots', 'timeSlotCounts', 'backloggedTrucks'))->with([
    ...
    public function getCompanyData($id)
    {
        ...
        $capabilities = collect(explode(',', $company->company_capabilities))
            ->filter()
        ...
        $specialties = collect(explode(',', $company->company_specialties))
            ->filter()
            ->map(function ($item, $index) {
                return [
                    'id' => $index + 1,
    ...
    public function acceptOrder(Request $request, $id)
    {
        $validated = $request->validate([
            ...
            'status' => 'required|string|in:Start,Loaded,Empty,End', // adjust valid statuses as per your app logic
        ]);
        ...
        $order = Order::where('order_number', $ticket['order_number'])->first();
        if (!$order) {
        ...
        $ticketAssign->user_id = $ticket->user_id;
        $ticketAssign->ticket_id = $id;
        $ticketAssign->truck_id = $validated['truck_id'];
        $ticketAssign->trailer_id = $validated['trailer_id']; // nullable
        $ticketAssign->status = $validated['status'];
        $ticketAssign->assigned_at = now();
        $ticketAssign->save();
